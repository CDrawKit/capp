...
{
	//We load two shaders, one is a solid-color shader, for most capp objects.
...
function CAPPEmitBackendTrianglesJS( vertsF, colorsI, vertcount )
{
...
let imports = {
	env: {
		...
		CAPPEmitBackendTriangles : (vertsF, colorsI, vertcount )=>
		{
			//Take a float* and uint32_t* of vertices, and flat-render them.
			CAPPEmitBackendTrianglesJS(
		...
		CAPPSetupFullscreen : (title,w,h ) => {
			SystemStart( title, w, h );
			canvas.style = "position:absolute; top:0; left:0;"
			fullscreen = true;
		},
		CAPPClearFrameInternal: ( color ) => {
...
if( CAPP_NEED_BLITTER )
{
	...
	imports.env.CAPPBlitImageInternal = (memptr, x, y, w, h ) => {
			if( w <= 0 || h <= 0 ) return;
...

{
	...
	WebAssembly.instantiate(array, imports).then(
		function(wa) { 
			...
			//Actually invoke main().  Note that, upon "CAPPSwapBuffers" this will 'exit'
