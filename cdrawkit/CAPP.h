// Copyright 2010-2021 <>< , et. al. (Several other authors, many but not all mentioned)
//	Licensed under the MIT/x11 or NewBSD License you choose.
//
//	CA Foundational Graphics Main Header File.  This is the main header you
//	should include.  See README.md for more details.


#ifndef _CAPP_H
#define _CAPP_H

#ifdef __cplusplus
extern "C" {
#endif

/* Rawdraw flags:
	CAPP3D -> Enable the weird 3D functionality that capp has to allow you to
		write apps which emit basic capp primitives but look 3D!
		CAPP_USE_DOUBLE_FUNCTIONS -> Use double-precision floating point for CAPP3D.
	CAPPOGL -> Use an OpenGL Backend for all capp functionality.
		->Caveat->If using CAPP_HAS_XSHAPE, then, we do something realy wacky.
	CAPPRASTERIZER -> Software-rasterize the capp calls, and, use
		CAPPUpdateScreenWithBitmap to send video to webpage.
	CAPPCONTEXTONLY -> Don't add any drawing functions, only opening a window to
		get an OpenGL context.

    CAPP_IMPLEMENTATION -> #define this and it will make _this_ the file where the capp
		functions actually live.
		
Usually tested combinations:
 * TCC On Windows and X11 (Linux) with:
    - CAPPOGL on or CAPPOGL off.  If CAPPOGL is off you can use
			CAPP_WINDOWS_DISABLE_BATCH to disable all batching.
		-or-
	- CAPPRASTERIZER

	NOTE: Sometimes you can also use CAPPOGL + CAPPRASTERIZER

 * WASM driver supports both: CAPPRASTERIZER and without CAPPRASTERIZER (Recommended turn rasterizer off)
 * ANDROID (But this automatically sets CAPPRASTERIZER OFF and CAPPOGL ON)

 Unusual compiler flags:

 * CAPPHTTP - Enable the HTTP server-version of capp, where it renders to a website.
 * CAPPHTTPSERVERONLY - if you want to use the HTTP server w/o capp.  You will need to implement:
	- CloseEvent, HTTPCustomCallback, HTTPCustomStart, NewWebSocket, WebSocketData, WebSocketTick
 * CAPP_DISABLE_HTTP_FILES - disable the HTTP file server.

*/


#include <stdint.h>

//Some per-platform logic.
#if defined( ANDROID ) || defined( __android__ )
	#define CAPPOGL
#endif

#if ( defined( CAPPOGL ) || defined( __wasm__ ) ) && !defined(CAPP_HAS_XSHAPE)

	#define CAPP_BATCH 8192 //131,072 bytes.

	#if defined( ANDROID ) || defined( __android__ ) || defined( __wasm__ ) || defined( EGL_LEAN_AND_MEAN )
		#define CAPPEWGL //EGL or WebGL
	#else
		#define CAPPDESKTOPGL
	#endif
#endif

typedef struct {
    short x, y; 
} RDPoint; 

extern int CAPPPenX, CAPPPenY;
extern uint32_t CAPPBGColor;
extern uint32_t CAPPLastColor;
extern uint32_t CAPPDialogColor; //Only used for DrawBox

//Draws text at CAPPPenX, CAPPPenY, with scale of `scale`.
void CAPPDrawText( const char * text, short scale );

//Determine how large a given test would be to draw.
void CAPPGetTextExtents( const char * text, int * w, int * h, int textsize  );

//Draws a box, outline as whatever the last CAPPColor was set to but also draws
//a rectangle as a background as whatever CAPPDialogColor is set to.
void CAPPDrawBox( short x1, short y1, short x2, short y2 );

//To be provided by driver. Rawdraw uses colors in the format 0xRRGGBBAA
//Note that some backends do not support alpha of any kind.
//Some platforms also support alpha blending.  So, be sure to set alpha to 0xFF
uint32_t CAPPColor( uint32_t RGBA );

//This both updates the screen, and flips, all as a single operation.
void CAPPUpdateScreenWithBitmap( uint32_t * data, int w, int h );


//This is only supported on a FEW architectures, but allows arbitrary
//image blitting.  Note that the alpha channel behavior is different
//on different systems.
void CAPPBlitImage( uint32_t * data, int x, int y, int w, int h );

// Only supported with CAPPOGL
#ifdef CAPPOGL
void CAPPDeleteTex( unsigned int tex );
unsigned int CAPPTexImage( uint32_t *data, int w, int h );
void CAPPBlitTex( unsigned int tex, int x, int y, int w, int h );
#endif

void CAPPTackPixel( short x1, short y1 );
void CAPPTackSegment( short x1, short y1, short x2, short y2 );
void CAPPTackRectangle( short x1, short y1, short x2, short y2 );
void CAPPTackPoly( RDPoint * points, int verts );
void CAPPClearFrame();
void CAPPSwapBuffers();

void CAPPGetDimensions( short * x, short * y );


//This will setup a window.  Note that w and h have special meaning. On Windows
//and X11, for instance if you set w and h to be negative, then capp will not
//show the window to the user.  This is useful if you just need it for some
//off-screen-rendering purpose.
//
//Return value of 0 indicates success.  Nonzero indicates error.
int CAPPSetup( const char * WindowName, int w, int h ); 

void CAPPSetupFullscreen( const char * WindowName, int screen_number );
int CAPPHandleInput();


//You must provide:
void HandleKey( int keycode, int bDown );
void HandleButton( int x, int y, int button, int bDown );
void HandleMotion( int x, int y, int mask );
void HandleDestroy();

//Internal function for resizing rasterizer for rasterizer-mode.
void CAPPInternalResize( short x, short y ); //don't call this.

//Not available on all systems.  Use The OGL portion with care.
#ifdef CAPPOGL
void   CAPPSetVSync( int vson );
void * CAPPGetExtension( const char * extname );
#endif

//Also not available on all systems.  Transparency.
void	CAPPPrepareForTransparency();
void	CAPPDrawToTransparencyMode( int transp );
void	CAPPClearTransparencyLevel();

//Only available on systems that support it.
void	CAPPSetLineWidth( short width );
void	CAPPChangeWindowTitle( const char * windowtitle );
void	CAPPSetWindowIconData( int w, int h, uint32_t * data );
int 	CAPPSetupWMClass( const char * WindowName, int w, int h , char * wm_res_name_ , char * wm_res_class_ );

//If you're using a batching renderer, for instance on Android or an OpenGL
//You will need to call this function inbetewen swtiching properties of drawing.  This is usually
//only needed if you calling OpenGL / OGLES functions directly and outside of CAPP.
//
//Note that these are the functions that are used on the backends which support this
//sort of thing.
#ifdef CAPP_BATCH

//If you are not using the CAPPOGL driver, you will need to define these in your driver.
void	CAPPEmitBackendTriangles( const float * vertices, const uint32_t * colors, int num_vertices );
void	CAPPBlitImage( uint32_t * data, int x, int y, int w, int h );

//These need to be defined for the specific driver.  
void 	CAPPClearFrame();
void 	CAPPSwapBuffers();

void 	CAPPFlushRender(); //Emit any geometry (lines, squares, polys) which are slated to be rendered.
void	CAPPInternalResize( short x, short y ); //Driver calls this after resize happens.
void	CAPPSetupBatchInternal(); //Driver calls this after setup is complete.

//Useful function for emitting a non-axis-aligned quad.
void 	CAPPEmitQuad( float cx0, float cy0, float cx1, float cy1, float cx2, float cy2, float cx3, float cy3 );

extern int 	CAPPVertPlace;
extern float CAPPVertDataV[CAPP_BATCH*3];
extern uint32_t CAPPVertDataC[CAPP_BATCH];
#endif


#if defined(WINDOWS) || defined(WIN32) || defined(WIN64) || defined(_WIN32) || defined(_WIN64)

#define CAPP_KEY_SHIFT 0x10
#define CAPP_KEY_BACKSPACE 0x08
#define CAPP_KEY_DELETE 0x2E
#define CAPP_KEY_LEFT_ARROW 0x25
#define CAPP_KEY_RIGHT_ARROW 0x27
#define CAPP_KEY_TOP_ARROW 0x26
#define CAPP_KEY_BOTTOM_ARROW 0x28
#define CAPP_KEY_ESCAPE 0x1B
#define CAPP_KEY_ENTER 0x0D

#elif defined( EGL_LEAN_AND_MEAN ) // doesn't have any keys
#elif defined( __android__ ) || defined( ANDROID ) // ^
#elif defined( __wasm__ )

#define CAPP_KEY_SHIFT 16
#define CAPP_KEY_BACKSPACE 8
#define CAPP_KEY_DELETE 46
#define CAPP_KEY_LEFT_ARROW 37
#define CAPP_KEY_RIGHT_ARROW 39
#define CAPP_KEY_TOP_ARROW 38
#define CAPP_KEY_BOTTOM_ARROW 40
#define CAPP_KEY_ESCAPE 27
#define CAPP_KEY_ENTER 13

#else // most likely x11

#define CAPP_KEY_SHIFT 65505
#define CAPP_KEY_BACKSPACE 65288
#define CAPP_KEY_DELETE 65535
#define CAPP_KEY_LEFT_ARROW 65361
#define CAPP_KEY_RIGHT_ARROW 65363
#define CAPP_KEY_TOP_ARROW 65362
#define CAPP_KEY_BOTTOM_ARROW 65364
#define CAPP_KEY_ESCAPE 65307
#define CAPP_KEY_ENTER 65293
#define CAPP_X11_EXPOSE 0xff00 //65280

#endif

#ifdef CAPP3D

#ifndef __wasm__
#include <math.h>
#endif

#ifdef CAPP_USE_DOUBLE_FUNCTIONS
#define tdCOS cos
#define tdSIN sin
#define tdTAN tan
#define tdSQRT sqrt
#else
#define tdCOS cosf
#define tdSIN sinf
#define tdTAN tanf
#define tdSQRT sqrtf
#endif

#ifdef __wasm__
void tdMATCOPY( float * x, const float * y ); //Copy y into x
#else
#define tdMATCOPY(x,y) memcpy( x, y, 16*sizeof(float))
#endif

#define tdQ_PI 3.141592653589
#define tdDEGRAD (tdQ_PI/180.)
#define tdRADDEG (180./tdQ_PI)


//General Matrix Functions
void tdIdentity( float * f );
void tdZero( float * f );
void tdTranslate( float * f, float x, float y, float z );		//Operates ON f
void tdScale( float * f, float x, float y, float z );			//Operates ON f
void tdRotateAA( float * f, float angle, float x, float y, float z ); 	//Operates ON f
void tdRotateQuat( float * f, float qw, float qx, float qy, float qz ); 	//Operates ON f
void tdRotateEA( float * f, float x, float y, float z );		//Operates ON f
void tdMultiply( float * fin1, float * fin2, float * fout );		//Operates ON f
void tdPrint( const float * f );
void tdTransposeSelf( float * f );

//Specialty Matrix Functions
void tdPerspective( float fovy, float aspect, float zNear, float zFar, float * out ); //Sets, NOT OPERATES. (FOVX=degrees)
void tdLookAt( float * m, float * eye, float * at, float * up );	//Operates ON m
//General point functions
#define tdPSet( f, x, y, z ) { f[0] = x; f[1] = y; f[2] = z; }
void tdPTransform( const float * pin, float * f, float * pout );
void tdVTransform( const float * vin, float * f, float * vout );
void td4Transform( float * kin, float * f, float * kout );
void td4RTransform( float * kin, float * f, float * kout );
void tdNormalizeSelf( float * vin );
void tdCross( float * va, float * vb, float * vout );
float tdDistance( float * va, float * vb );
float tdDot( float * va, float * vb );
#define tdPSub( x, y, z ) { (z)[0] = (x)[0] - (y)[0]; (z)[1] = (x)[1] - (y)[1]; (z)[2] = (x)[2] - (y)[2]; }
#define tdPAdd( x, y, z ) { (z)[0] = (x)[0] + (y)[0]; (z)[1] = (x)[1] + (y)[1]; (z)[2] = (x)[2] + (y)[2]; }

//Stack Functionality
#define tdMATRIXMAXDEPTH 32
extern float * gSMatrix;
void tdPush();
void tdPop();
void tdMode( int mode );
#define tdMODELVIEW 0
#define tdPROJECTION 1

//Final stage tools
void tdSetViewport( float leftx, float topy, float rightx, float bottomy, float pixx, float pixy );
void tdFinalPoint( float * pin, float * pout );

float tdNoiseAt( int x, int y );
float tdFLerp( float a, float b, float t );
float tdPerlin2D( float x, float y );

#endif

extern const unsigned char RawdrawFontCharData[1405];
extern const unsigned short RawdrawFontCharMap[256];

#ifdef __cplusplus
};
#endif


#if defined( ANDROID ) || defined( __android__ )
#include "CAPPAndroid.h"
#endif

#ifdef CAPP_IMPLEMENTATION
#include "CAPP.c"
#endif

#endif

