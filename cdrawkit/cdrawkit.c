...
//#define CAPP_WINDOWS_DISABLE_BATCH
...
//		CAPPColor(((x+y)&1)?0xFFFFFFFF:0x000000FF);

		float bright = tdDot( normal, lightdir );
		if( bright < 0 ) bright = 0;
		CAPPColor( 0xff | (((int)( bright * 50 ))<<24) );
...
//		CAPPTackSegment( ptc[0], ptc[1], ptd[0], ptd[1] );
//		CAPPTackSegment( ptd[0], ptd[1], pta[0], pta[1] );
		CAPPTackSegment( pta[0], pta[1], ptc[0], ptc[1] );
//		CAPPTackSegment( ptd[0], ptd[1], ptb[0], ptb[1] );
...
int main()
{
	...
	CAPPBGColor = 0x000080FF; //Darkblue
...
*/
		...
		CAPPColor( 0xffffffff );
		CAPPDialogColor = 0x202020ff;
		CAPPDrawBox( 300, 0, 320, 20 );

		// Square behind text
		CAPPColor( 0x444444FF );
