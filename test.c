...
//		CAPPColor(((x+y)&1)?0xFFFFFF:0x000000);

		float bright = tdDot( normal, lightdir );
		if( bright < 0 ) bright = 0;
		CAPPColor( 0xff | ( ( (int)( bright * 90 ) ) << 24 ) );

//		CAPPTackPoly( &pto[0], 3 );		CAPPTackPoly( &pto[3], 3 );
		CAPPTackSegment( pta[0], pta[1], ptb[0], ptb[1] );
		CAPPTackSegment( pta[0], pta[1], ptc[0], ptc[1] );
		CAPPTackSegment( ptb[0], ptb[1], ptc[0], ptc[1] );
...
int main( int argc, char ** argv )
{
	...
	CAPPBGColor = 0x000040ff;
	CAPPSetupFullscreen( "Test Bench", 0 );
	//CAPPSetup( "Test Bench", 0, 0 );
	...
	while(1)
	{
		...
		CAPPSetLineWidth( 9 );
		DrawHeightmap();
		CAPPPenX = 0; CAPPPenY = 400;
		CAPPColor( 0xffffffff );
		CAPPDrawText( assettext, 15 );
		CAPPFlushRender();

		CAPPPenX = 0; CAPPPenY = 480;
		char st[50];
...
*/
		...
		// Square behind text
		CAPPColor( 0x303030ff );
		CAPPTackRectangle( 600, 0, 950, 350);
		...
		CAPPBlitImage( randomtexturedata, 100, 600, 256, 256 );

		frames++;
		//On Android, CAPPSwapBuffers must be called, and CAPPUpdateScreenWithBitmap does not have an implied framebuffer swap.
		CAPPSwapBuffers();
